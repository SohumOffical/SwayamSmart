...
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.ColorHelper
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.DrawableHelper
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.dip
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.getDrawableCompat
...
internal class CellImageView : AppCompatImageView {
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        ...
        try {
            a?.apply {
                isBitmap = getBoolean(R.styleable.CellImageView_imageview_isBitmap, isBitmap)
                useColor = getBoolean(R.styleable.CellImageView_imageview_useColor, useColor)
                resource = getResourceId(R.styleable.CellImageView_imageview_resource, resource)
                color = getColor(R.styleable.CellImageView_imageview_color, color)
                size = getDimensionPixelSize(R.styleable.CellImageView_imageview_size, size)
                actionBackgroundAlpha = getBoolean(R.styleable.CellImageView_imageview_actionBackgroundAlpha, actionBackgroundAlpha)
                changeSize = getBoolean(R.styleable.CellImageView_imageview_changeSize, changeSize)
                fitImage = getBoolean(R.styleable.CellImageView_imageview_fitImage, fitImage)
            }
        } finally {
            a?.recycle()
        }
