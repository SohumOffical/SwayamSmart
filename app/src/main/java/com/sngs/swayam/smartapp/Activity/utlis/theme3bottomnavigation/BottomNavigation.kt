...
package com.naimee.swayam.utlis.theme3bottomnavigation
...
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.dip
import com.sngs.swayam.smartapp.Activity.utlis.theme3bottomnavigation.dipf
import com.sngs.swayam.smartapp.R


internal typealias IBottomNavigationListener = (model: BottomNavigation.Model) -> Unit

@Suppress("MemberVisibilityCanBePrivate")
class BottomNavigation : FrameLayout {

    var models = ArrayList<Model>()
    var cells = ArrayList<BottomNavigationCell>()
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(attrs, R.styleable.BottomNavigation, 0, 0)
        try {
            a?.apply {
                defaultIconColor = getColor(R.styleable.BottomNavigation_bn_defaultIconColor, defaultIconColor)
                selectedIconColor = getColor(R.styleable.BottomNavigation_bn_selectedIconColor, selectedIconColor)
                backgroundBottomColor = getColor(R.styleable.BottomNavigation_bn_backgroundBottomColor, backgroundBottomColor)
                countTextColor = getColor(R.styleable.BottomNavigation_bn_countTextColor, countTextColor)
                countBackgroundColor = getColor(R.styleable.BottomNavigation_bn_countBackgroundColor, countBackgroundColor)
                val typeface = getString(R.styleable.BottomNavigation_bn_countTypeface)
                rippleColor = getColor(R.styleable.BottomNavigation_bn_rippleColor, rippleColor)
                shadowColor = getColor(R.styleable.BottomNavigation_bn_shadowColor, shadowColor)
    ...
    fun add(model: Model) {
        val cell = BottomNavigationCell(context)
        cell.apply {
            val params = LinearLayout.LayoutParams(0, heightCell, 1f)
            layoutParams = params
            icon = model.icon
            count = model.count
            circleColor = this@BottomNavigation.backgroundBottomColor
            ...
            rippleColor = this@BottomNavigation.rippleColor
    ...
    private fun anim(cell: BottomNavigationCell, id: Int, enableAnimation: Boolean = true) {
        isAnimating = true
    ...
    fun getCellById(id: Int): BottomNavigationCell? {
    ...
    class Model(var id: Int, var icon: Int) {
        var count: String = BottomNavigationCell.EMPTY_VALUE
    }
}